...
namespace ListingsManager.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(IUnitOfWork uowInstance)
            : this(new UserManager<ApplicationUser, Guid>(new UserStore<ApplicationUser, CustomRole, Guid, CustomUserLogin, CustomUserRole, CustomUserClaim>(uowInstance as DbContext)))
        {
            uow = uowInstance;
        }
        ...
        public AccountController(UserManager<ApplicationUser, Guid> userManager)
        {
            ...
            UserManager.UserValidator = new UserValidator<ApplicationUser, Guid>(UserManager)
            {
        ...
        public ActionResult AccountInTrash(Guid id)
        {
            ...
            ChangeAccountStatus(id, AccountStatus.InTrash);
            return PartialView("_AccountInTrashPartial", user);
        }

        public ActionResult DeleteAccount(Guid id, bool getQuestion = true)
        {
            var users = uow.UserRepository;
            var user = users.Get(id);
            if (getQuestion)
            {
                return PartialView("_AccountDeleteQuestionPartial", user);
            }

            ChangeAccountStatus(id, AccountStatus.Deleted);
            return PartialView("_UserDeletedPartial", user);
        }
        ...
        public void ChangeAccountStatus(Guid id, AccountStatus newStatus)
        {
            var users = uow.UserRepository;
            var user = users.Get(id);
        ...
        public ActionResult AccountAdministration(string sortBy, int? page, int? pageCount, string status)
        {
            ...
            var users = uow.UserRepository.GetAll()
                       .Where(
                           u => u.AccountStatus == accountStatus && u.AccountType != AccountType.Administrator);
            switch (sortBy)
            {
                case "Name":
            ...
            return View(users.ToList().ToPagedList(page ?? 1, pageCount ?? 20));
        }

        public ActionResult MakePremium(Guid id)
        {
